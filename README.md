# Hi 👋, I'm Salma Abdulkader
**"NTI Graduate | Junior Data Scientist"**

🌱 I’m passionate about Machine Learning, Deep Learning, Computer Vision, and NLP  
⚡ I love coding, experimenting, and building AI-powered projects  
🤝 I’m looking to collaborate on interesting AI and Data Science projects  
✉️ Contact me at: [salma.ahmd.k1678@gmail.com](mailto:salma.ahmd.k1678@gmail.com)

---

## 🌐 Connect with me

- LinkedIn: [Salma Abdulkader](https://www.linkedin.com/in/salmaabdulkader)  
- GitHub: [salmaahmed2828](https://www.github.com/salmaahmed2828)  
- Instagram: [salma_abdulkader_](https://www.instagram.com/salma_abdulkader_)  
- Facebook: [Salma Facebook](https://www.facebook.com/share/17D6T1f9ua/)

---

## 💻 Technical Stack

### 🖥️ Programming Languages
Python | C++ | Java  

### 🤖 Machine Learning
Scikit-Learn | NumPy | Pandas | Matplotlib | Seaborn | OpenCV  

### 🧠 Deep Learning
TensorFlow | PyTorch | Keras | HuggingFace Transformers  

### 🚀 Deployment & MLOps
Streamlit | Flask | FastAPI | Docker | MLFlow | DVC | CI/CD  

### 📊 Data Analysis & Business Intelligence
SQL | MySQL | Excel | Power BI | Statistics | Time Series Analysis  

### 👾 Operating Systems
Linux | Ubuntu | Windows

---

## 🛠️ Projects & Contributions

- **Image Captioning with Transformers** – Generate captions for product images using BLIP model. [GitHub Link](https://github.com/salmaahmed2828/Image-Captioning)  
- **Head Pose Estimation** – Predict head pose angles from images using CNN and 3D annotations. [GitHub Link](https://github.com/salmaahmed2828/Head-Pose-Estimation)  
- **SPR X-Ray Age Prediction** – Predict age from X-Ray images using deep learning. [GitHub Link](https://github.com/salmaahmed2828/SPR-XRay-Age)

---

[![](https://visitcount.itsvg.in/api?id=salmaahmed2828&icon=4&color=1)](https://visitcount.itsvg.in)
